...
$("#btnStartCal").click(function () {
    if ($("#txtHtmlRawData").val() !== "") {
        ...
        var inputHTMLContent = $("#txtHtmlRawData").val();
        inputHTMLContent = inputHTMLContent.replaceAll("<script", "<script>");
        inputHTMLContent = inputHTMLContent.replaceAll("<link", "<link>");
        newModelData = newModelData.replace("AppendPageContent", inputHTMLContent);
        $(".container").append(newModelData);

        $("#result").empty();
        $("#result").append("<br><h4><strong>Below are all the tags with there counts:</strong><br /></h4>");
